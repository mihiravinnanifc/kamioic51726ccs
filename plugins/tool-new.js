...
cmd({
    ...
    react: "🕒",
    desc: "Get current time and date of any city/country",
    category: "utility"
}, async (conn, mek, m, { reply, args }) => {
    // පරිශීලකයා නමක් ලබා දී නැත්නම් default ශ්‍රී ලංකාව පෙන්වමු
    const location = args[0] ? args.join(" ") : "Colombo";
    const apiKey = 'zuRyqX3oWW8ZhRO68B3Wc8xwwugfMGUfGWTCCQmd'; // ඔයා ලබාදුන් API Key එක

    try {
        // API-Ninjas World Time API එකට රික්වෙස්ට් එක යැවීම
        ...
        if (data && data.datetime) {
            // වෙලාව සහ දිනය ලස්සනට format කරමු
            let msg = `🕒 *WORLD CLOCK SERVICE* 🕒\n\n` +
            ...
            reply("❌ *Invalid Location!* කරුණාකර රටක හෝ නගරයක නම නිවැරදිව ලබා දෙන්න.");
        }

    } catch (e) {
        // API එකේ අවුලක් හෝ වැරදි නමක් දුන් විට
        reply("❌ දත්ත ලබාගැනීමේදී දෝෂයක් සිදුවුණා. කරුණාකර පසුව උත්සාහ කරන්න.");
        console.error(e);
    }
});
